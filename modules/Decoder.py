...
class Decoder(nn.Module, ABC):
    ...
    def rencos_beam_search(self, beam_size: int, max_dec_step: int, BeamClass, src_encodings, src_lens, last_state, last_cell,
        ...
        while (not beam.is_finished) and cur_step < max_dec_step:
            ...
            _src_encodings, _src_lens = beam.expand_static_input((src_encodings.permute(1, 0, 2), src_lens))
            _src_encodings_0, _src_lens_0 = beam.expand_static_input((src_encodings_0.permute(1, 0, 2), src_lens_0))
            _src_encodings_1, _src_lens_1 = beam.expand_static_input((src_encodings_1.permute(1, 0, 2), src_lens_1))

            att_t, alpha_t, state_t, cell_t, decoder_output = self.step(y_tm1_embed, att_tm1, _src_encodings.permute(1, 0, 2), _src_lens, last_state, last_cell)
            att_t_0, alpha_t_0, state_t_0, cell_t_0, decoder_output_0 = self.step(y_tm1_embed, att_tm1_0, _src_encodings_0.permute(1, 0, 2), _src_lens_0, last_state_0, last_cell_0)
            att_t_1, alpha_t_1, state_t_1, cell_t_1, decoder_output_1 = self.step(y_tm1_embed, att_tm1_1, _src_encodings_1.permute(1, 0, 2), _src_lens_1, last_state_1, last_cell_1)
            prob_input = self.prepare_prob_input(decoder_output)
